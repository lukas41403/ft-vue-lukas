<template>
    ...
    <div class="flex justify-center">
        <div class="w-full max-w-screen-xl mx-auto p-4">
            <h1 class="text-2xl font-bold mb-2 mt-4 px-8">LED reťaze</h1>
            <div class="flex flex-wrap justify-between">
                <div v-for="(item, index) in cartStore.products" :key="index"
                    ...
                    <div class="image-container rounded-lg overflow-hidden">
                        <img class="product-image" :src="'/product_images/' + item.picture_1" :alt="item.name">
                    </div>
                    <div class="flex justify-between items-center mt-2">
                        <h3 class="text-lg font-semibold">{{ item.name }}</h3>
                        <h4 class="text-md">Cena: {{ item.price }} €</h4>
                    </div>
                    <div>
                        <h6 class="text-sm">Dĺžka: {{ item.size }} m</h6>
...
export default {
    ...
    created() {
        this.cartStore.loadProducts('retaze')
    }
}
</script>